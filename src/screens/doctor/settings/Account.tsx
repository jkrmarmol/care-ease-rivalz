...
import { getInfoAccount } from "../../../store/doctor/settingSlice";

export default function Account() {
  const dispatch = useAppDispatch();
  const { width, height } = useWindowDimensions();
  const nav = useNavigation<NavigationProp<ParamListBase>>();
  const selectGetInfoAccount = useAppSelector((state) => state.doctorSetting.getInfoAccount);
  const onPressLogout = () => {
    dispatch(logout());
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {selectGetInfoAccount.status === "ok" &&
            `${selectGetInfoAccount.response.firstName} ${selectGetInfoAccount.response.lastName}`}
        </Text>
        <Text
          style={{
            fontFamily: "KalekoBold",
        ...
        >
          {selectGetInfoAccount.status === "ok" && selectGetInfoAccount.response.user.role}
        </Text>
        <View
          style={{
            flex: 1,
        ...
        >
          ...
          >
            ...
            <View>
              {/* <TouchableOpacity
                onPress={() => nav.navigate("DoctorChangeInformation")}
                style={{
                  flexDirection: "row",
                  alignItems: "center",
              ...
              <TouchableOpacity
                onPress={() => nav.navigate("DoctorChangePassword")}
                style={{
                  flexDirection: "row",
                  alignItems: "center",
                  justifyContent: "space-between",
